...
def fetch_medal_tally(df, years, country):
    medal_df = df.drop_duplicates(subset=['NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'])
    flag = 0
...
def most_successful(df, sport):
    ...
    f_df = pd.merge(temp_df, x, on='Name')

    final_df = f_df.sort_values('count', ascending=False).drop_duplicates('Name').head(15)[
        ['Name', 'count', 'Sport', 'region']]
    final_df.rename(columns={'count': 'Medals'}, inplace=True)
...
def most_successful_athletes_country_wise(df, country):
    ...
    final_df = f_df.sort_values('count', ascending=False).drop_duplicates('Name').head(10)[['Name', 'count', 'Sport']]
    final_df.rename(columns={'count': 'Medals'}, inplace=True)
